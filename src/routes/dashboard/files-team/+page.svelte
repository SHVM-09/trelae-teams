<script lang="ts">
	...
	async function loadFiles() {
		const res = await fetch(`/api/files?visibility=team`);
		const data = await res.json();
		// Filter by current location only
		teamFiles = data.files.filter((f: File) => f.location === currentLocation);
	}
	...
	async function confirmDelete() {
		...
		teamFiles = teamFiles.filter(f => f.id !== fileToDelete?.id);
	...
	async function confirmBulkDelete() {
		...
		teamFiles = teamFiles.filter(f => !selectedIds.includes(f.id));
	...
	async function searchFiles(query: string) {
		...
		const res = await fetch(`/api/search?q=${encodeURIComponent(query)}&visibility=team`);
		const data = await res.json();
...
<svelte:head>
	<title>Team Files - Trelae Teams</title>
	<meta name="description" content="Manage your team's files securely in a shared space." />
			...
			<div class="flex flex-wrap justify-end gap-2">
				...
				{#if moveFileId}
					<Button
						...
						disabled={teamFiles.find(f => f.id === moveFileId)?.location === currentLocation}
						aria-label="Move file here"
						title={teamFiles.find(f => f.id === moveFileId)?.location === currentLocation
